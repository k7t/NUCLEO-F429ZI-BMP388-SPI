...
//	//HAL_UART_Transmit_DMA(&huart3, buff, size);
...
int8_t SensorAPI_SPIx_Read(uint8_t subaddress, uint8_t *pBuffer,
	...
	//HAL_SPI_TransmitReceive(&hspi3, pTxData, pRxData, ReadNumbr+1, BUS_TIMEOUT); // timeout 1000msec;
...
int8_t SensorAPI_SPIx_Write(uint8_t subaddress, uint8_t *pBuffer,
	...
	//HAL_SPI_TransmitReceive(&hspi3, pTxData, pRxData, WriteNumbr+1, BUS_TIMEOUT); // send register address + write data
